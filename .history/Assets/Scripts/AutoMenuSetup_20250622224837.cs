    ...
    public class AutoMenuSetup : MonoBehaviour
    {
        ...
        public void CreateCompleteMenuSystem()
        {
            ...
            try
            {
                ...
                Debug.Log("🎯 Step 5: Creating menu manager AFTER panels exist...");
                CreateMenuManager();
        ...
        {
            Debug.Log("🎮 Creating SimpleMenuSystem...");
            ...
            if (existingManager != null)
            {
                ...
                Debug.Log("🗑️ Removed existing MenuManager");
            }

                        GameObject managerObj = new GameObject("MenuManager");
            SimpleMenuSystem simpleMenu = managerObj.AddComponent<SimpleMenuSystem>();

            Debug.Log("✅ SimpleMenuSystem created!");
            Debug.Log("🔄 Initializing SimpleMenuSystem now that panels exist...");

            simpleMenu.InitializeMenuSystem();

            Debug.Log("✅ SimpleMenuSystem fully initialized!");
        }
        ...
        {
            Debug.Log("🔗 Button events will be connected automatically by SimpleMenuSystem!");
            Debug.Log("✅ No manual connection needed - SimpleMenuSystem handles everything!");
        }
        ...
        private void ValidateCompleteSetup()
        {
            ...
            GameObject canvas = GameObject.Find("MenuCanvas");
            if (canvas == null) {
                Debug.LogError("❌ RED X RED - ERROR #4: MenuCanvas not found!");
                isValid = false;
                errorCount++;
            }
            else {
                ...
                GameObject gameplayPanel = GameObject.Find("GameplayPanel");
                                ...
                                if (mainPanel == null) { Debug.LogError("❌ RED X RED - ERROR #1: MainMenuPanel not found!"); errorCount++; }
                else { Debug.Log("✅ MainMenuPanel found"); }

                if (modelPanel == null) { Debug.LogError("❌ RED X RED - ERROR #2: ModelSelectionPanel not found!"); errorCount++; }
                else { Debug.Log("✅ ModelSelectionPanel found"); }

                if (gameplayPanel == null) { Debug.LogError("❌ RED X RED - ERROR #3: GameplayPanel not found!"); errorCount++; }
                else { Debug.Log("✅ GameplayPanel found"); }
            }

            GameObject manager = GameObject.Find("MenuManager");
            if (manager == null) {
                Debug.LogError("❌ RED X RED - ERROR #5: MenuManager not found!");
                isValid = false;
                errorCount++;
            }
            else {
            ...
            SimpleMenuSystem simpleMenu = FindObjectOfType<SimpleMenuSystem>();
            if (simpleMenu == null) {
                Debug.LogError("❌ RED X RED - ERROR #6: SimpleMenuSystem component not found!");
                isValid = false;
                errorCount++;
            }
            else {
                Debug.Log("✅ SimpleMenuSystem component found");

                Debug.Log("🔍 Checking menu panel references:");
                Debug.Log($"   - mainMenuPanel: {(simpleMenu.mainMenuPanel != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - modelSelectionPanel: {(simpleMenu.modelSelectionPanel != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - gameplayPanel: {(simpleMenu.gameplayPanel != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - currentState: {simpleMenu.currentState}");

                if (simpleMenu.mainMenuPanel == null) { Debug.LogError("❌ RED X RED - ERROR #7: SimpleMenuSystem.mainMenuPanel is NULL!"); errorCount++; }
                if (simpleMenu.modelSelectionPanel == null) { Debug.LogError("❌ RED X RED - ERROR #8: SimpleMenuSystem.modelSelectionPanel is NULL!"); errorCount++; }
            }

            GameObject originPoint = GameObject.Find("RecenterOriginPoint");
            if (originPoint == null) {
                Debug.LogError("❌ RED X RED - ERROR #9: RecenterOriginPoint not found!");
                errorCount++;
            }
            else {
            ...
            if (errorCount == 0) {
                ...
                Debug.LogError("🔍 DETAILED ERROR BREAKDOWN:");
                Debug.LogError($"   - Missing components or references detected");
                Debug.LogError($"   - Check the individual error messages above for specifics");
                Debug.LogError($"   - Total error count: {errorCount}");
            }
        }
        ...
        {
            Debug.Log("🔍 MANUAL VALIDATION STARTED");
            ValidateCompleteSetup();
        }

        [ContextMenu("🔍 Quick Debug - Find Missing Components")]
        public void QuickDebugMissingComponents()
        {
            Debug.Log("🔍🔍🔍 QUICK DEBUG - SEARCHING FOR MISSING COMPONENTS 🔍🔍🔍");
            ...
            Debug.Log("🔍 GameObject Search Results:");
            Debug.Log($"   MenuCanvas: {(canvas != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   MainMenuPanel: {(mainPanel != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   ModelSelectionPanel: {(modelPanel != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   GameplayPanel: {(gameplayPanel != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   MenuManager: {(manager != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   IniciarButton: {(iniciarBtn != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   SalirButton: {(salirBtn != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   BackButton: {(backBtn != null ? "✅ FOUND" : "❌ MISSING")}");
            Debug.Log($"   RecenterOriginPoint: {(originPoint != null ? "✅ FOUND" : "❌ MISSING")}");

            // Check components
            SimpleMenuSystem simpleMenu = FindObjectOfType<SimpleMenuSystem>();
            Debug.Log($"   SimpleMenuSystem component: {(simpleMenu != null ? "✅ FOUND" : "❌ MISSING")}");

            if (simpleMenu != null)
            {
                Debug.Log("🔍 SimpleMenuSystem Panel References:");
                Debug.Log($"   mainMenuPanel: {(simpleMenu.mainMenuPanel != null ? "✅ ASSIGNED" : "❌ NULL")}");
                Debug.Log($"   modelSelectionPanel: {(simpleMenu.modelSelectionPanel != null ? "✅ ASSIGNED" : "❌ NULL")}");
                Debug.Log($"   gameplayPanel: {(simpleMenu.gameplayPanel != null ? "✅ ASSIGNED" : "❌ NULL")}");
                Debug.Log($"   currentState: {simpleMenu.currentState}");
            }

            FurnitureVisibilityController[] controllers = FindObjectsOfType<FurnitureVisibilityController>();
            Debug.Log($"   FurnitureVisibilityControllers: {controllers.Length} found");
