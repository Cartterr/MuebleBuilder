    ...
    public class AutoMenuSetup : MonoBehaviour
    {
        ...
        public void CreateCompleteMenuSystem()
        {
            ...
            try
            {
                ...
                Debug.Log("🎯 Step 3: Setting up furniture visibility...");
                SetupFurnitureVisibility();
                Debug.Log("✅ Furniture visibility setup completed");
                ...
                Debug.Log("🎯 Step 7: Setting up furniture selection...");
                SetupFurnitureSelection();
                Debug.Log("✅ Furniture selection setup completed");

                Debug.Log("🎯 Step 8: Connecting button events...");
                ConnectButtonEvents();
        ...
        {
            CreateText("MuebleBuilder", parent, new Vector2(0, 0.5f), 0.1875f);
            CreatePhysicalButton("Mueble1Button", "MUEBLE 1", parent, new Vector2(0, 0.15f), new Vector2(0.75f, 0.2f));
            CreatePhysicalButton("Mueble2Button", "MUEBLE 2", parent, new Vector2(0, -0.05f), new Vector2(0.75f, 0.2f));
            CreatePhysicalButton("Mueble3Button", "MUEBLE 3", parent, new Vector2(0, -0.25f), new Vector2(0.75f, 0.2f));
            CreatePhysicalButton("SalirButton", "SALIR", parent, new Vector2(0, -0.45f), new Vector2(0.75f, 0.2f));
        }
        ...
        public void CreateMenuManager()
        {
            ...
            StandardMenuSystem standardSystem = managerObj.AddComponent<StandardMenuSystem>();
            ...
            Debug.Log("✅ Created StandardMenuSystem and FurnitureSelectionManager");
        }
        ...
        {
            Debug.Log("🔗 Starting button event connection...");

            StandardMenuSystem standardSystem = FindObjectOfType<StandardMenuSystem>();
            if (standardSystem != null)
            {
                Debug.Log("✅ Found StandardMenuSystem - using standard Unity approach!");
                Debug.Log("🔗✅ StandardMenuSystem will handle all button events automatically!");
                return;
            }

            Debug.Log("⚠️ No StandardMenuSystem found, falling back to FurnitureSelectionManager...");

            FurnitureSelectionManager selectionManager = FindObjectOfType<FurnitureSelectionManager>();
            if (selectionManager == null)
            {
                Debug.LogError("❌ RED X RED - No FurnitureSelectionManager found!");
                return;
            }
            Debug.Log($"✅ Found FurnitureSelectionManager: {selectionManager.name}");

            Debug.Log($"🔍 Button availability check:");
            Debug.Log($"   - iniciarButton: {(selectionManager.iniciarButton != null ? "✅ Found" : "❌ RED X RED - NULL")}");
            Debug.Log($"   - salirButton: {(selectionManager.salirButton != null ? "✅ Found" : "❌ RED X RED - NULL")}");
            Debug.Log($"   - backToMenuButton: {(selectionManager.backToMenuButton != null ? "✅ Found" : "❌ RED X RED - NULL")}");

            if (selectionManager.iniciarButton != null)
            {
                selectionManager.iniciarButton.onClick.RemoveAllListeners();
                selectionManager.iniciarButton.onClick.AddListener(() => {
                    Debug.Log("🚀🚀🚀 INICIAR BUTTON CLICKED! 🚀🚀🚀");
                    Debug.Log("🔄 Calling TransitionToModelSelection...");
                    selectionManager.TransitionToModelSelection();
                });
                Debug.Log("✅ INICIAR button event connected successfully!");
            }
            else
            {
                Debug.LogError("❌ RED X RED - INICIAR button is NULL! Cannot connect event!");
            }

            if (selectionManager.salirButton != null)
            {
                selectionManager.salirButton.onClick.RemoveAllListeners();
                selectionManager.salirButton.onClick.AddListener(() => {
                    Debug.Log("👋👋👋 SALIR BUTTON CLICKED! 👋👋👋");
                    selectionManager.ExitApplication();
                });
                Debug.Log("✅ SALIR button event connected successfully!");
            }
            else
            {
                Debug.LogError("❌ RED X RED - SALIR button is NULL! Cannot connect event!");
            }

            if (selectionManager.backToMenuButton != null)
            {
                selectionManager.backToMenuButton.onClick.RemoveAllListeners();
                selectionManager.backToMenuButton.onClick.AddListener(() => {
                    Debug.Log("🔙🔙🔙 BACK BUTTON CLICKED! 🔙🔙🔙");
                    selectionManager.ShowMainMenu();
                });
                Debug.Log("✅ BACK button event connected successfully!");
            }
            else
            {
                Debug.LogError("❌ RED X RED - BACK button is NULL! Cannot connect event!");
            }

            Debug.Log("🔗✅ Button event connection process completed!");
        }
        ...
        private void ValidateCompleteSetup()
        {
            ...
            if (canvas == null) {
                ...
                if (modelPanel == null) { Debug.LogError("❌ RED X RED - ModelSelectionPanel not found!"); errorCount++; }
                else { Debug.Log("✅ ModelSelectionPanel found"); }

                if (gameplayPanel == null) { Debug.LogError("❌ RED X RED - GameplayPanel not found!"); errorCount++; }
                else { Debug.Log("✅ GameplayPanel found"); }
            }
            ...
            FurnitureSelectionManager selectionManager = FindObjectOfType<FurnitureSelectionManager>();
            if (selectionManager == null) {
                Debug.LogError("❌ RED X RED - FurnitureSelectionManager component not found!");
                isValid = false;
                errorCount++;
            }
            else {
                Debug.Log("✅ FurnitureSelectionManager component found");

                Debug.Log("🔍 Checking UI references:");
                Debug.Log($"   - mainMenuAnimator: {(selectionManager.mainMenuAnimator != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - modelSelectionAnimator: {(selectionManager.modelSelectionAnimator != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - gameplayAnimator: {(selectionManager.gameplayAnimator != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - iniciarButton: {(selectionManager.iniciarButton != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - salirButton: {(selectionManager.salirButton != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - backToMenuButton: {(selectionManager.backToMenuButton != null ? "✅" : "❌ RED X RED")}");
                Debug.Log($"   - modelGridParent: {(selectionManager.modelGridParent != null ? "✅" : "❌ RED X RED")}");

                if (selectionManager.mainMenuAnimator == null) errorCount++;
                if (selectionManager.modelSelectionAnimator == null) errorCount++;
                if (selectionManager.iniciarButton == null) errorCount++;
            }
