    ...
    public class AutoMenuSetup : MonoBehaviour
    {
        ...
        private Mesh CreateOriginIndicatorMesh()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                vertices[i + 4] = new Vector3(x, height, z);
            }
            ...
            {
                int next = (i + 1) % 4;
                int baseIndex = 12 + i * 6;

                triangles[baseIndex] = i;
                triangles[baseIndex + 1] = i + 4;
                triangles[baseIndex + 2] = next;

                triangles[baseIndex + 3] = next;
                triangles[baseIndex + 4] = i + 4;
                triangles[baseIndex + 5] = next + 4;
            }
