...
namespace VRProject
{
    ...
    public class AutoMenuSetup : MonoBehaviour
    {
        ...
        public List<InstructionData> instructionDatabase = new List<InstructionData>
        {
            ...
            new InstructionData { objectName = "furniture_1_3_spawned", instructionText = "Conecta esta pata a una esquina de la base de la mesa", displayDuration = 4f },
            new InstructionData { objectName = "furniture_1_4_spawned", instructionText = "Conecta esta pata a una esquina de la base de la mesa", displayDuration = 4f },
            new InstructionData { objectName = "furniture_1_5_spawned", instructionText = "Conecta esta pata a una esquina de la base de la mesa", displayDuration = 4f },
        ...
        public void CreateCompleteMenuSystem()
        {
            ...
            try
            {
                ...
                Debug.Log("🎯 Step 8: Creating smart furniture instruction manager...");
                CreateFurnitureInstructionManager();
                Debug.Log("✅ Smart furniture instruction manager created");
                ...
                private void CreateInstructionPanelIfNotExists()
        {
            ...
            if (playerSpawnPoint != null)
            {
                panelRoot.transform.position = playerSpawnPoint.position + Vector3.forward * 0.5f + Vector3.up * 1.5f;
            }
            else
            {
                panelRoot.transform.position = new Vector3(0, 1.5f, 0.5f);
            }

            panelRoot.transform.localScale = Vector3.one * 0.0025f;
            ...
            scaler.scaleFactor = 0.000000125f;
            ...
            canvasRect.sizeDelta = new Vector2(0.08f, 0.015f);

            GameObject backgroundPanel = CreateBeautifulPanel("BackgroundPanel", panelRoot.transform);
            ...
            bgRect.sizeDelta = new Vector2(0.08f, 0.015f);
            ...
            textRect.anchorMin = Vector2.zero;
            textRect.anchorMax = Vector2.one;
            textRect.offsetMin = new Vector2(0.006f, 0.002f);
            textRect.offsetMax = new Vector2(-0.006f, -0.002f);
            textRect.anchoredPosition3D = new Vector3(0, 0, -0.001f);

            TextMeshProUGUI instructionText = textContainer.AddComponent<TextMeshProUGUI>();
            instructionText.text = "";
            instructionText.fontSize = 0.006f;
            ...
            Debug.Log("✨ Beautiful instruction panel created with fade animations!");
                ...
                private GameObject CreateBeautifulPanel(string name, Transform parent)
        {
            GameObject panel = new GameObject(name);
            panel.transform.SetParent(parent);
            ...
            Texture2D beautifulTexture = CreateBeautifulGradientTexture(120, 30);
            Sprite beautifulSprite = Sprite.Create(beautifulTexture, new Rect(0, 0, beautifulTexture.width, beautifulTexture.height), new Vector2(0.5f, 0.5f));

            image.sprite = beautifulSprite;
            image.type = Image.Type.Simple;

            return panel;
        }

        private Texture2D CreateBeautifulGradientTexture(int width, int height)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Color topColor = new Color(0.2f, 0.4f, 0.8f, 0.95f);
                    Color bottomColor = new Color(0.1f, 0.2f, 0.6f, 0.95f);
                    Color gradientColor = Color.Lerp(bottomColor, topColor, normalizedY);

                    float alpha = 1f;

                    float distanceFromLeft = x;
                    float distanceFromRight = width - 1 - x;
                    float distanceFromTop = y;
                    float distanceFromBottom = height - 1 - y;

                    if (distanceFromLeft < cornerRadius && distanceFromTop < cornerRadius)
                    {
                        float distance = Vector2.Distance(new Vector2(x, y), new Vector2(cornerRadius, cornerRadius));
                        alpha = distance <= cornerRadius ? 1f : 0f;
                    }
                    else if (distanceFromRight < cornerRadius && distanceFromTop < cornerRadius)
                    {
                        float distance = Vector2.Distance(new Vector2(x, y), new Vector2(width - 1 - cornerRadius, cornerRadius));
                        alpha = distance <= cornerRadius ? 1f : 0f;
                    }
                    else if (distanceFromLeft < cornerRadius && distanceFromBottom < cornerRadius)
                    {
                        float distance = Vector2.Distance(new Vector2(x, y), new Vector2(cornerRadius, height - 1 - cornerRadius));
                        alpha = distance <= cornerRadius ? 1f : 0f;
                    }
                    else if (distanceFromRight < cornerRadius && distanceFromBottom < cornerRadius)
                    {
                        float distance = Vector2.Distance(new Vector2(x, y), new Vector2(width - 1 - cornerRadius, height - 1 - cornerRadius));
                        alpha = distance <= cornerRadius ? 1f : 0f;
                    }

                    float glowIntensity = 1f - (Vector2.Distance(new Vector2(normalizedX, normalizedY), new Vector2(0.5f, 0.5f)) * 0.3f);
                    gradientColor.r += glowIntensity * 0.1f;
                    gradientColor.g += glowIntensity * 0.1f;
                    gradientColor.b += glowIntensity * 0.1f;
            ...
            return texture;
                ...
                [ContextMenu("🎯 Create Smart Furniture Instruction Manager")]
        public void CreateFurnitureInstructionManager()
        {
            GameObject existingManager = GameObject.Find("FurnitureInstructionManager");
            if (existingManager != null)
            {
                Debug.Log("✅ FurnitureInstructionManager already exists, checking components...");

                FurnitureInstructionManager existingComponent = existingManager.GetComponent<FurnitureInstructionManager>();
                if (existingComponent == null)
                {
                    Debug.Log("🔧 Adding missing FurnitureInstructionManager component");
                    existingComponent = existingManager.AddComponent<FurnitureInstructionManager>();
                    existingComponent.instructionDatabase = this.instructionDatabase;
                }
                else
                {
                    Debug.Log("✅ Updating instruction database on existing manager");
                    existingComponent.instructionDatabase = this.instructionDatabase;
                }
                return;
            }

            Debug.Log("🔄 Creating new FurnitureInstructionManager...");

            GameObject managerObj = new GameObject("FurnitureInstructionManager");
            FurnitureInstructionManager manager = managerObj.AddComponent<FurnitureInstructionManager>();
            manager.instructionDatabase = this.instructionDatabase;
            ...
            {
                manager.instructionPanelController = instructionController;
                Debug.Log("🔗 Connected instruction panel to furniture manager");
            }

            Debug.Log("✅ FurnitureInstructionManager created and configured!");
        }

        [ContextMenu("🧹 Clean Old Furniture Components")]
        public void CleanOldFurnitureComponents()
        {
            Debug.Log("🧹 Cleaning old furniture components...");

            FurnitureGrabDetector[] grabDetectors = Resources.FindObjectsOfTypeAll<FurnitureGrabDetector>();
            foreach (FurnitureGrabDetector detector in grabDetectors)
            {
                if (detector != null && detector.gameObject != null)
                {
                    ...
                    Debug.Log($"🗑️ Removed FurnitureGrabDetector from {detector.gameObject.name}");
                }
            }

            Debug.Log($"✅ Cleaned {grabDetectors.Length} old grab detectors");
        }

        [ContextMenu("🗑️ REMOVE ALL OLD INSTRUCTION COMPONENTS")]
        public void RemoveAllOldInstructionComponents()
        {
            Debug.Log("🗑️🗑️🗑️ REMOVING ALL OLD INSTRUCTION COMPONENTS 🗑️🗑️🗑️");

            // Remove old FurnitureInstructionManager objects
            GameObject[] oldManagers = GameObject.FindObjectsOfType<GameObject>()
                .Where(obj => obj.name.Contains("FurnitureInstructionManager"))
                .ToArray();

            foreach (GameObject manager in oldManagers)
            {
                Debug.Log($"🗑️ Removing old FurnitureInstructionManager: {manager.name}");
                if (Application.isPlaying)
                    Destroy(manager);
                else
            ...
            // Remove old InstructionPanel objects
            GameObject[] oldPanels = GameObject.FindObjectsOfType<GameObject>()
                .Where(obj => obj.name.Contains("InstructionPanel"))
                .ToArray();

            foreach (GameObject panel in oldPanels)
            {
                Debug.Log($"🗑️ Removing old InstructionPanel: {panel.name}");
                if (Application.isPlaying)
                    Destroy(panel);
                else
            ...
            // Clean old grab detectors
            CleanOldFurnitureComponents();

            Debug.Log("✅✅✅ ALL OLD INSTRUCTION COMPONENTS REMOVED! ✅✅✅");
            Debug.Log("🔄 Now run 'CREATE COMPLETE MENU SYSTEM' to create fresh components with Spanish instructions!");
        }
    }
}