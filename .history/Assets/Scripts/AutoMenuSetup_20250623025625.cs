    ...
    public class AutoMenuSetup : MonoBehaviour
    {
        ...
        public void CleanOldFurnitureComponents()
        {
            GameObject[] allFurniture = GameObject.FindObjectsOfType<GameObject>()
                .Where(obj => obj.name.StartsWith("furniture_") && !obj.name.Contains("_spawned"))
                .ToArray();

            Debug.Log($"🧹 Cleaning old components from {allFurniture.Length} original furniture objects...");

            int removedCount = 0;

            foreach (GameObject furniture in allFurniture)
            {
                FurnitureGrabDetector oldDetector = furniture.GetComponent<FurnitureGrabDetector>();
                if (oldDetector != null)
                {
                    if (Application.isPlaying)
                        Destroy(oldDetector);
                    ...
                    Debug.Log($"🗑️ Removed FurnitureGrabDetector from {furniture.name}");
                    removedCount++;
                }

                Button oldButton = furniture.GetComponent<Button>();
                if (oldButton != null)
                {
                    if (Application.isPlaying)
                        Destroy(oldButton);
                    ...
                    Debug.Log($"🗑️ Removed Button from {furniture.name}");
                }

                FurnitureTracker oldTracker = furniture.GetComponent<FurnitureTracker>();
                if (oldTracker != null)
                {
                    if (Application.isPlaying)
                        Destroy(oldTracker);
                    ...
                    Debug.Log($"🗑️ Removed FurnitureTracker from {furniture.name}");
                }
            }

            Debug.Log($"✅ Cleanup completed! Removed components from {removedCount} furniture objects");
            Debug.Log("🎯 The new FurnitureInstructionManager will handle everything dynamically!");
        }
    }
}