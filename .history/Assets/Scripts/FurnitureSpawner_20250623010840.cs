    ...
    public class FurnitureSpawner : MonoBehaviour
    {
        ...
        public void SpawnFurnitureSet(string setNumber)
        {
            ...
            foreach (FurnitureData data in furnitureToSpawn)
            {
                if (data.prefab != null)
                {
                    // Temporarily enable the original to ensure spawned copy is active
                    bool originalState = data.prefab.activeSelf;
                    ...
                    // Restore original state and ensure spawned is active
                    data.prefab.SetActive(originalState);
                    spawnedObj.SetActive(true);

                    currentSpawnedFurniture.Add(spawnedObj);
                    spawnedCount++;

                    Debug.Log($"   🏗️ Spawned: {data.name} at {data.position} - Active: {spawnedObj.activeSelf}");
                }
                else
                {
