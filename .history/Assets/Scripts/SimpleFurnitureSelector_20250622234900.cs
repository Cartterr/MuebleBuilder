...
namespace VRProject
{
    public class SimpleFurnitureSelector : MonoBehaviour
    {
                ...
                private System.Collections.IEnumerator InitializeWithDelay()
        {
            ...
            // Wait a bit more to ensure all FurnitureVisibilityControllers have finished their Start()
            yield return new WaitForSeconds(0.5f);

            Debug.Log("🔄 Ensuring all furniture is properly hidden after controllers initialize...");
            ForceHideAllFurniture();
            HideRestartButton();
                        ...
                        private void HideAllFurniture()
        {
            ...
            Debug.Log($"🙈 Hid {controllerCount} furniture via controllers");

            GameObject[] allObjects = Resources.FindObjectsOfTypeAll<GameObject>();
            int directCount = 0;
            foreach (GameObject obj in allObjects)
            {
                if (obj.name.StartsWith("furniture_") && obj.scene.IsValid())
                {
            ...
            Debug.Log($"🙈 Hid {directCount} furniture directly");

            Debug.Log($"🙈 All furniture hidden (Controllers: {controllerCount}, Direct: {directCount})");
        }

        private void ForceHideAllFurniture()
        {
            Debug.Log("💪 FORCE hiding all furniture (overriding any other state)...");
            ...
            foreach (GameObject obj in allObjects)
            {
                if (obj.name.StartsWith("furniture_") && obj.scene.IsValid())
                {
                    // Force hide regardless of controller state
                    obj.SetActive(false);
                    totalHidden++;

                    // Also ensure the controller knows it's hidden
                    FurnitureVisibilityController controller = obj.GetComponent<FurnitureVisibilityController>();
                    if (controller != null)
                    {
                        // The controller's HideFurniture method just sets SetActive(false), so this is redundant but ensures consistency
                        controller.HideFurniture();
                    }
                }
            }

            Debug.Log($"💪 Force hidden {totalHidden} furniture pieces");
        }
                        ...
                        private void ShowFurnitureSet(string setPrefix)
        {
            ...
            foreach (GameObject obj in allObjects)
            {
                if (obj.name.StartsWith(setPrefix + "_") && obj.scene.IsValid())
                {
                    // Force show the object first
                    obj.SetActive(true);
                    shownCount++;
                    Debug.Log($"   👁️ Force showed: {obj.name}");

                    // Also tell the controller it's shown (if it exists)
                    FurnitureVisibilityController controller = obj.GetComponent<FurnitureVisibilityController>();
                    if (controller != null)
                    {
                        controller.ShowFurniture();
                        Debug.Log($"   👁️ Controller also notified for: {obj.name}");
                    }
                }
            }
            ...
            if (shownCount == 0)
            {
                Debug.LogWarning($"⚠️ No furniture found with prefix: {setPrefix}");

                Debug.Log("🔍 Searching for all furniture objects...");
                foreach (GameObject obj in allObjects)
                {
                    if (obj.name.Contains("furniture") && obj.scene.IsValid())
                    {
                        Debug.Log($"   🪑 Found furniture: {obj.name} - Active: {obj.activeSelf}");
                    }
                }
            }
            else
