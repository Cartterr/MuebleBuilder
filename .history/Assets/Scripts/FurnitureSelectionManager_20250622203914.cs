...
namespace VRProject
{
    public class FurnitureSelectionManager : MonoBehaviour
    {
                ...
                private void CreateFurnitureButton(FurnitureSet furnitureSet, int index)
        {
            ...
            buttonMaterial.color = new Color(0.3f, 0.5f, 0.8f, 1f);
            buttonMaterial.SetFloat("_Metallic", 0.3f);
            buttonMaterial.SetFloat("_Glossiness", 0.7f);
        ...
        private void HideAllFurniture()
        {
            if (furnitureSets.Count > 0)
            {
                foreach (FurnitureSet set in furnitureSets)
                {
                    foreach (GameObject piece in set.pieces)
                    {
                        if (piece != null)
                        {
                            FurnitureVisibilityController controller = piece.GetComponent<FurnitureVisibilityController>();
                            if (controller != null)
            ...
            {
                FurnitureVisibilityController[] allControllers = FindObjectsOfType<FurnitureVisibilityController>();
                foreach (FurnitureVisibilityController controller in allControllers)
                {
                    controller.HideFurniture();
                }
                Debug.Log($"🙈 Hid {allControllers.Length} furniture pieces using FurnitureVisibilityController");
            }
        }
        ...
        public void TransitionToModelSelection()
        {
            ...
            Debug.Log($"🔄 Transitioning to Model Selection. Current furniture sets: {furnitureSets.Count}");
            ...
            HideCurrentPanel(() => {
                ...
                {
                    Debug.LogError("❌ Model selection animator is null!");
